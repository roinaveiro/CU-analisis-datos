...
* En el filtrado, solo se incluyen filas con valores `TRUE` de la condición (se incluyen pues los `FALSE` y los `NA`).
...
Calcula la proporción de gente en cada continente, que en el año 1967 tenía esperanza de vida entre 
...
gapminder %>%
  ...
  mutate(prop = n / sum(n)) %>% slice(1:10)
```
